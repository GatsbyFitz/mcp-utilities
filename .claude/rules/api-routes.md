...
- Everything under `/api/` requires a session. `middleware.ts` denies by default and only `PUBLIC_API_PREFIXES` (NextAuth's own endpoints) is exempt, so a new route is protected because it exists. Still add the `getToken` check inside the handler as well — every existing route does, and the middleware is the backstop, not the only gate. `/mcp` sits outside `/api/` and stays public for external MCP clients.
