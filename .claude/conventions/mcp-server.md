...
Response *quality* is audited separately by [/qa-tool-responses](../skills/qa-tool-responses/SKILL.md): `/verify-mcp-server` proves a tool responds, that one grades what it responded with — citation truthfulness, retrieval relevance, response hygiene — against a live server. A daily Claude Code routine runs it and files regressions to the `MCP tool-response QA` tracking issue.
