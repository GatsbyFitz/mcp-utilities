...
- Cut the contextual-retrieval prefix, **if the log says it is not already cached**. `contextualizeChunks` sends the whole document on every call and varies only the trailing chunk, so its input is roughly the document times the chunk count — comfortably the largest model spend in the pipeline, larger than `createMarkdown` and `extractGraph` together. The first call is made alone so Gemini's implicit prefix cache can populate before the rest fan out, and every ingestion now logs `N prompt tokens, M of them cached (X%)` from `cachedContentTokenCount`, so this is answerable from the logs rather than by guessing. A high cached share means this item is done and needs nothing further. A low one, or a line saying the provider reported no counts, is the case that needs work: either explicit caching, or replacing the whole document with a bounded window — title, section headings, and the neighbouring chunks — which would cut the input by an order of magnitude but changes what the context says and therefore what retrieval does, so it needs the eval harness first
- Cap the chunk text a search tool returns, not just what it sends the reranker. `capForRerank` stops an oversized chunk failing the call, but the response still inlines it whole: one 58,000-character table is ~15,000 tokens in a single result. A cap with a "truncated — fetch the rest with …" marker needs the drill-down tools above to exist first, otherwise it hides exactly the reference table the query was aiming at
- Give text results the same page-anchored source link figures now have. A figure result links to `…pdf#page=64`; a prose result still links to the document with no page, so a citation reading "pp. 10–11" leaves the reader to find them. `Citation` already carries `pageStart` and `citationLine` already prints the range, so this is rendering only — no re-ingest, no metadata change. Left out when figures got it because the figure case had a demonstrated failure and this one does not
