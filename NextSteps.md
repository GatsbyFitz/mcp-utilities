...
- Measure and then cut the contextual-retrieval prefix. `contextualizeChunks` sends the whole document on every call and varies only the trailing chunk, so its input is roughly the document times the chunk count — comfortably the largest model spend in the pipeline, larger than `createMarkdown` and `extractGraph` together. Warming the prefix cache with a single first call is already done and costs nothing, but **nobody has checked the bill to see whether implicit caching is actually firing**, and that is the first thing to establish, since everything below is wasted work if it is. If it is not: either move to explicit caching, or replace the whole document with a bounded window — title, section headings, and the neighbouring chunks — which would cut the input by an order of magnitude but changes what the context says and therefore what retrieval does, so it needs the eval harness first
