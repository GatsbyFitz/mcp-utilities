...
**Only figures that carry information are kept.** The prompt names what to ignore — logos, letterheads, watermarks, stamps, signatures, page furniture, decorative photographs, barcodes — with the test being whether anyone answering a question about the document would ever need to look at it. Behind that sits a size backstop: anything covering less than `MIN_FIGURE_AREA` (3%) of the page is dropped, since a header logo is under 1% of a page while a process diagram is well into double figures, and the check runs *before* padding so the margin cannot lift a logo over the bar. A decorative mark is **dropped outright rather than sent to the whole-page fallback** — the two failures want opposite handling. An unusable box means a real figure was mislocated, and a whole page is worth more than a dropped diagram; a logo means there was nothing to crop, and falling back there would turn it into a full-page "figure" that gets embedded, stored and returned to a model as if it answered something.
...
### Chunking, and the table problem

[lib/chunking.ts](lib/chunking.ts) splits along Markdown block boundaries rather than a blind word count, packing blocks greedily to `size` words and starting a new chunk at each heading, so a chunk always carries its own heading context.

**A table used to bypass that budget entirely.** However long it runs, a Markdown table is one block, so a 400-row obligations table arrived as a single ~58,000-character chunk against ~2,000 for prose. Keeping it whole is right in principle — splitting a table changes what it means — but the cost fell everywhere at once: one embedding covering every row, so retrieval could not tell which row answered the query; one contextualisation call with the whole table as input; and the whole thing inlined into any tool response that retrieved it, which is what produced the 32,000-character rerank failure in production.

An oversized table is now split **between its rows, never within one** — a row is the table's unit of meaning — with the header and alignment row repeated on every part, so each part is still a readable table rather than a grid of unlabelled cells. Parts are packed to the same `size` budget as prose, and each reports the pages its *own* rows sit on, so a table running across a page break cites the page the matched rows are actually on. A table with fewer than three rows, or one row that alone busts the budget, has nothing safe to split on and is kept whole exactly as before. Measured on a synthetic 400-row table: 16 parts, longest chunk 3,154 characters against 49,468.

**This moves chunk IDs, so it needs a full re-ingest, not a re-embed.** `chunkId` is `${fileName}-${index}`, and every Neo4j relationship stores one to fetch its excerpt from Upstash — so until a document is re-ingested under the new boundaries, its graph hits resolve against the old IDs. Doing this before the corpus grows is much cheaper than after.
...
**In ingestion**, `contextualizeChunks` dominates everything else. It sends the *whole document* as the prefix of every call and varies only the chunk at the end, so its input is roughly the document times the chunk count — at ~80 chunks that is about 80× the document per ingestion, far more than `createMarkdown` or `extractGraph`. The prefix is byte-identical across those calls, which is exactly what Gemini's implicit prefix caching is for, but a cache only helps against a request it has already seen: firing five identical-prefix calls at once into a cold cache pays full price for all five. The first call is therefore made alone, before the rest fan out, which costs one call's latency on a step that already takes minutes. Whether that works is not left to belief: every ingestion logs `N prompt tokens, M of them cached (X%)`, read from the `cachedContentTokenCount` Gemini returns beside `promptTokenCount`. Implicit caching is otherwise invisible from the call site — a cached request and an uncached one are indistinguishable apart from the bill — and this is the number that decides whether the much larger change (replacing the whole document with a bounded window) is worth making at all. The lookup searches every provider-metadata key rather than assuming `google`, since these calls go through the AI Gateway; if nothing reports counts, the log says the question is unanswered rather than implying either answer.
